...
class cvTheque extends Component {
	...
	handleUploadSuccess = (filename) => {
		...
		firebase.storage().ref('newsletter').child(filename).getDownloadURL().then((url) =>
			this.setState({
				imageURL : url
			})
		);
	...
	onSubmit = (event, authUser, place) => {
		...
		const placeRef = db.collection('newsletter').add({
				...
				<form className="cvthequeForm" onSubmit={this.onSubmit}>
					<div className="block">
						...
						<FileUploader
							...
							name="image"
							storageRef={firebase.storage().ref('newsletter')}
